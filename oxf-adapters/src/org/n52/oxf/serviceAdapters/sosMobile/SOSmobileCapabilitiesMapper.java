 ...
 * 
 */
public class SOSmobileCapabilitiesMapper {

    private static Logger LOGGER = LoggingHandler.getLogger(SOSmobileCapabilitiesMapper.class);
            ...
            for (int j = 0; j < xb_parameters.length; j++) {
                ...
                if (parameterName.equalsIgnoreCase("eventTime")) {
                    ...
                    if (xb_allowedValues != null) {
                        RangeType[] xb_range = xb_allowedValues.getRangeArray();

                        ValueType xb_max = null;
                        ValueType xb_min = null;
                        for (int k = 0; k < xb_range.length; k++) {
                            RangeType xb_value = xb_range[k];
                            xb_max = xb_value.getMaximumValue();
                            xb_min = xb_value.getMinimumValue();

                            // int comparison = xb_max.compareValue(xb_min);
                            // System.out.println(comparison);
                        }

                        if (xb_max != null && xb_min != null) {
                            TimePeriod oc_timePeriod = new TimePeriod(xb_min.getStringValue(), xb_max.getStringValue());
                            IDiscreteValueDomain<ITime> oc_temporalDomain = new TemporalValueDomain(oc_timePeriod);

                            Parameter oc_parameter = new Parameter(parameterName, true, oc_temporalDomain, null);
    ...
    private SOSContents mapContents(CapabilitiesDocument capabilitiesDoc) throws OXFException {
        ...
        for (int i = 0; i < xb_obsOfferings.length; i++) {
            ...
            if (oc_crs != null) {
                ...
                // TODO daniel: take away break once the envelope is correct!
                LOGGER.error("crs is null!",
                             new NullPointerException("SRS not specified in the Capabilities' 'gml:Envelope'-tag."));
                break;
                // throw new NullPointerException(
                // "SRS not specified in the Capabilities' 'gml:Envelope'-tag.");
            }

            // BoundingBox:
            List<Double> xb_lowerCornerList = xb_obsOffering.getBoundedBy().getEnvelope().getLowerCorner().getListValue();
            double[] oc_lowerCornerList = new double[xb_lowerCornerList.size()];

            List<Double> xb_upperCornerList = xb_obsOffering.getBoundedBy().getEnvelope().getUpperCorner().getListValue();
            double[] oc_upperCornerList = new double[xb_upperCornerList.size()];

            for (int j = 0; j < xb_lowerCornerList.size(); j++) {
                oc_lowerCornerList[j] = xb_lowerCornerList.get(j).doubleValue();
            }
            for (int j = 0; j < xb_upperCornerList.size(); j++) {
                oc_upperCornerList[j] = xb_upperCornerList.get(j).doubleValue();
            }
            xb_lowerCornerList = null;
            xb_upperCornerList = null;
