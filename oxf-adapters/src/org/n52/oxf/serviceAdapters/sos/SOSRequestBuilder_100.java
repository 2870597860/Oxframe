...
import net.opengis.ogc.IdDocument;
...
import net.opengis.swe.x10.BooleanPropertyType;
import net.opengis.swe.x10.SimpleDataPropertyType;
import net.opengis.swe.x10.SimpleDataRecordDocument;
import net.opengis.swe.x10.SimpleDataRecordPropertyType;
...
//    	measurementCur.toNextToken();
    ...
    public String buildRegisterSensor(ParameterContainer parameters) throws OXFException{
    	...
    	if(parameters.getParameterShellWithCommonName(ISOSRequestBuilder.REGISTER_SENSOR_ML_DOC_PARAMETER)!= null){
	    	...
	    	Identification identification=system.addNewIdentification();  //添加的
	    	IdentifierList identificationList=identification.addNewIdentifierList(); //添加的
	    	Identifier identifier=identificationList.addNewIdentifier();
	    	//TermDocument termDocument=TermDocument.Factory.newInstance();
	    	Term term=Term.Factory.newInstance();
	    	term.setValue((String) parameters.getParameterShellWithCommonName(ISOSRequestBuilder.REGISTER_SENSOR_IDENTIFICATION_TYPE).getSpecifiedValue());
	    	term.setDefinition("urn:ogc:def:identifier:OGC:uniqueID"); 
	    	identifier.setTerm(term);
	    	
	    	Capabilities capabilites=system.addNewCapabilities();  
	    	...
	    	Inputs inputs = system.addNewInputs(); //添加inputs元素
	    	InputList inputList=inputs.addNewInputList(); //添加InputsList元素     
	    	ObservableProperty observableProperty=inputList.addNewInput().addNewObservableProperty(); //添加
	    	observableProperty.setDefinition("urn:ogc:def:phenomenon:OGC:1.0.30:mood");
	    	 
	    	Outputs outputs = system.addNewOutputs(); //添加outputs
	    	 OutputList outputList = outputs.addNewOutputList(); //添加outputList
	    	                        
	    	 
	    	/*Quantity quantity=outputList.addNewOutput().addNewQuantity();
	    	quantity.setDefinition("urn:ogc:def:phenomenon:OGC:1.0.30:waterlevel");
	    	quantity.addNewUom().setCode("cm");
	    	*/ //可用
	    	/*
	    	Text text=outputList.addNewOutput().addNewText(); //添加Text元素 
	     	text.setDefinition("urn:ogc:def:phenomenon:OGC:1.0.30:patientCondition");
	     	MetaDataPropertyType metaDataPropertyType=text.addNewMetaDataProperty();  //gml:MetaDataProperty 
	     	RegisterSensor regSensor1 = regSensorDoc.addNewRegisterSensor();
	     	regSensor1.setOffering((String)parameters.getParameterShellWithCommonName(ISOSRequestBuilder.REGISTER_SENSOR_OBSERVED_PROPERTY_PARAMETER).getSpecifiedValue());
	     	*/
	     	
	     	
	     	/*IoComponentPropertyType IoCompProp = outputList.addNewOutput(); //output元素
	     	IoCompProp.setName("text"); 
	     	*/
	     	/*Quantity quantity = IoCompProp.addNewQuantity();
	    	quantity.setDefinition("urn:ogc:def:identifier:OGC:people");
	     	...
	     	*/
	     	...
	     	/*Text text=IoCompProp.addNewText();
	     	text.setDefinition("urn:ogc:def:phenomenon:OGC:1.0.30:patientCondition");
	     	MetaDataPropertyType metaDataPropertyType=text.addNewMetaDataProperty();  //gml:MetaDataProperty
	     	metaDataPropertyType.set(metaDataPropertyType);
	     	AbstractIdType abstractIdType=AbstractIdType.Factory.newInstance();
	      */
	     	
	     	
	     	ObservationOfferingList observationOfferingList=ObservationOfferingList.Factory.newInstance();
	     	  
	     	observationOfferingList.addNewObservationOffering();
